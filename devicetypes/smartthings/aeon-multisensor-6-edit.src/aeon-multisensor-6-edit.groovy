...
metadata {
	definition(name: "Aeon Multisensor 6 Edit", namespace: "smartthings", author: "SmartThings", runLocally: true, minHubCoreVersion: '000.020.00008', executeCommandsLocally: true, ocfDeviceType: "x.com.st.d.sensor.motion") {
	...
	preferences {
    	input "tempOffset", "enum", title: "Temperature Offest", description: "Fix Reported Deg", options: ["-1 deg", "-2 deg", "-3 deg", "+1 deg", "+2 deg", "+3 deg"], displayDuringSetup: true
		input "motionDelayTime", "enum", title: "Motion Sensor Delay Time", options: ["20 seconds", "40 seconds", "1 minute", "2 minutes", "3 minutes", "4 minutes"]
		input "motionSensitivity", "enum", title: "Motion Sensor Sensitivity", options: ["maximum", "normal", "minimum", "disabled"]
		input "reportInterval", "enum", title: "Report Interval", description: "How often the device should report in minutes", options: ["8 minutes", "15 minutes", "30 minutes", "1 hour", "6 hours", "12 hours", "18 hours", "24 hours"]
	}
	...
	tiles(scale: 2) {
		...
		valueTile("temperature2", "device.temperature", inactiveLabel: false, width: 6, height: 4) {
...
["-1 deg", "-2 deg", "-3 deg", "+1 deg", "+2 deg", "+3 deg"]
private def getOffsetOptionValueMap() {
	[
		"-1 deg": -1,
		"-2 deg": -2,
		"-3 deg": -3,
